...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if([keyPath isEqualToString:@"estimatedProgress"])
    {
		...
		if ([self.delegate respondsToSelector:@selector(webView:progressChange:)]) {
			[self.delegate webView:self progressChange:self.estimatedProgress];
		}
    }
    else if([keyPath isEqualToString:@"title"])
    {
		...
		if ([self.delegate respondsToSelector:@selector(webView:titleChange:)]) {
			[self.delegate webView:self titleChange:self.title];
		}
    }
